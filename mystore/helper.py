import tempfile
from .models import StoreStatus,  ReportStatus, StoreStatusLog, StoreTimezone
...
def trigger_report_combined(report):
    ...
    # only triggering report for first 100 stores.
    stores = StoreTimezone.objects.all()[:150]
    for store in stores:
        # print(store)
...
def generate_report_data(store):
    ...
    # data for uptime and downtime for last hour, last day and last week
    data = []
    data.append(store.pk)
    data.append(last_one_hour_data['uptime'])
    data.append(last_one_day_data['uptime'])
    data.append(last_one_week_data['uptime'])
    data.append(last_one_hour_data['downtime'])
    data.append(last_one_day_data['downtime'])
    data.append(last_one_week_data['downtime'])
